...
import { TransactionWrapper, Contract, KyberPriceFeed, Engine } from '..';
...
export async function calculateAmgu(
  ...
  const prices = new KyberPriceFeed(priceSourceAddress, provider);
  const engine = new Engine(engineAddress, provider);
